...
import { new1EnemyAt, new1EnemyCount, type New1Pose } from "../../new1Reel";
...
export function New1Foes({ soldiers }: New1FoesProps) {
  ...
  useFrame((state) => {
    ...
    skip.current += 1;
    if (cap > 3600 && recT > 2.1 && skip.current % 2 === 1) return;
    const n = cap;
    for (let i = 0; i < n; i++) {
      new1EnemyAt(i, soldiers, recT, scratch);
