...
import { NEW5_FOES, NEW6_FOES, NEW62_FOES, new1EnemyAt, new1EnemyCount, new2EnemyAt, new2VisualFriends, new5EnemyAt, new5FoeCount, new6EnemyAt, type New1Pose } from "../../new1Reel";
...
export function New1Foes({ soldiers, duel = false, swords = false, bridge = false, cross = false, wide = false, packed = false }: New1FoesProps) {
  ...
  const cap = wide ? NEW62_FOES : cross ? NEW6_FOES : packed ? new5FoeCount(soldiers) : bridge ? NEW5_FOES : Math.max(1, new1EnemyCount(fightFriends));
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      else if (bridge) new5EnemyAt(i, recT, scratch, packed ? soldiers : 0);
