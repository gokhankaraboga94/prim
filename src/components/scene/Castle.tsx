...
function useStoneTexture(enabled = true) {
  return useMemo(() => {
...
function Block({
  ...
  return (
    <mesh position={position} rotation={rotation} castShadow={!lite} receiveShadow={!lite}>
...
function SquareTower({
  ...
  return (
    <group position={position}>
      <Block args={[base, height * 0.22, base]} position={[0, height * 0.11, 0]} color={STONE_2} map={map} lite={lite} />
      <Block args={[size, height, size]} position={[0, height / 2, 0]} color={STONE} map={map} lite={lite} />
      <Block args={[size + 0.22, 0.7, size + 0.22]} position={[0, height * 0.62, 0]} color={STONE_2} map={map} lite={lite} />
      <Block args={[size + 0.36, 0.32, size + 0.36]} position={[0, top, 0]} color={STONE_3} map={map} lite={lite} />
      {!lite && (
        <>
...
function Gatehouse({
  ...
  return (
    <group position={[0, 0, 6.85]}>
      <Block args={[1.7, wallH + 2.05, 3.15]} position={[-2.25, (wallH + 2.05) / 2, 0.2]} color={STONE} map={stone} lite={lite} />
      <Block args={[1.7, wallH + 2.05, 3.15]} position={[2.25, (wallH + 2.05) / 2, 0.2]} color={STONE} map={stone} lite={lite} />
      <Block args={[6.4, 1.55, 3.3]} position={[0, wallH + 1.35, 0.22]} color={STONE_2} map={stone} lite={lite} />
      <Block args={[6.8, 0.32, 3.55]} position={[0, wallH + 2.2, 0.26]} color={STONE_3} map={stone} lite={lite} />
      {!lite && (
        <>
      ...
      <mesh position={[0, 2.55, 1.42]} rotation={[Math.PI / 2, 0, 0]}>
        ...
        {lite ? <meshBasicMaterial color={STONE_3} /> : <meshStandardMaterial color={STONE_3} map={stone ?? undefined} />}
      </mesh>
      <Block args={[2.7, 0.32, 0.55]} position={[0, 2.52, 1.42]} color={STONE_2} map={stone} lite={lite} />
...
function RoundTower({
  ...
  const merlonN = lite ? 0 : 10;
  return (
    <group position={position}>
      <mesh position={[0, height / 2, 0]}>
        <cylinderGeometry args={[radius, radius + 0.12, height, lite ? 8 : 16]} />
        {lite ? <meshBasicMaterial color={STONE_2} /> : <meshStandardMaterial color={STONE_2} map={map ?? undefined} />}
      </mesh>
      <mesh position={[0, height + 0.08, 0]}>
        <cylinderGeometry args={[radius + 0.16, radius + 0.1, 0.22, lite ? 8 : 10]} />
        {lite ? <meshBasicMaterial color={STONE_3} /> : <meshStandardMaterial color={STONE_3} />}
      </mesh>
...
export function Castle({ level, gateClosed = false, forceGateOpen = false, lite = false }: CastleProps) {
  ...
  return (
    ...
    <group position={[0, 0, zShift]} scale={[sx, sy, sz]}>
      <mesh position={[0, 0.05, -4.05]} receiveShadow={!lite}>
        <boxGeometry args={[21.6, 0.08, 18.2]} />
        {lite ? (
          <meshBasicMaterial color="#6a6258" />
        ) : (
      ...
      <Block args={[24.4, wallH, 2.15]} position={[0, wallH / 2, -13.2]} color={STONE} map={stone} lite={lite} />
      <Block args={[9.4, wallH, 2.15]} position={[-6.7, wallH / 2, 6.55]} color={STONE} map={stone} lite={lite} />
      <Block args={[9.4, wallH, 2.15]} position={[6.7, wallH / 2, 6.55]} color={STONE} map={stone} lite={lite} />
      <Block args={[2.15, wallH, 21.6]} position={[-10.85, wallH / 2, -3.325]} color={STONE} map={stone} lite={lite} />
      <Block args={[2.15, wallH, 21.6]} position={[10.85, wallH / 2, -3.325]} color={STONE} map={stone} lite={lite} />
      <Block args={[21.6, 0.22, 0.85]} position={[0, wallH - 0.05, -12.35]} color={STONE_3} map={stone} lite={lite} />
      <Block args={[8.2, 0.22, 0.85]} position={[-6.7, wallH - 0.05, 5.7]} color={STONE_3} map={stone} lite={lite} />
      <Block args={[8.2, 0.22, 0.85]} position={[6.7, wallH - 0.05, 5.7]} color={STONE_3} map={stone} lite={lite} />
      <Block args={[0.85, 0.22, 19.6]} position={[-10.05, wallH - 0.05, -3.325]} color={STONE_3} map={stone} lite={lite} />
      <Block args={[0.85, 0.22, 19.6]} position={[10.05, wallH - 0.05, -3.325]} color={STONE_3} map={stone} lite={lite} />
      ...
      <Gatehouse wallH={wallH} stone={stone} closed={gateClosed} forceOpen={forceGateOpen} lite={lite} />
      <mesh position={[0, wallH * 0.72, -13.05]}>
        <boxGeometry args={[0.2, 0.62, 0.1]} />
        {lite ? <meshBasicMaterial color={STONE_DARK} /> : <meshStandardMaterial color={STONE_DARK} />}
      </mesh>

      <Block args={[1.15, wallH * 0.72, 1.15]} position={[-7.2, wallH * 0.36, 4.1]} color={STONE_2} map={stone} lite={lite} />
      <Block args={[1.15, wallH * 0.72, 1.15]} position={[7.2, wallH * 0.36, 4.1]} color={STONE_2} map={stone} lite={lite} />
      <Block args={[1.15, wallH * 0.72, 1.15]} position={[-7.2, wallH * 0.36, -10.4]} color={STONE_2} map={stone} lite={lite} />
      <Block args={[1.15, wallH * 0.72, 1.15]} position={[7.2, wallH * 0.36, -10.4]} color={STONE_2} map={stone} lite={lite} />

      <SquareTower position={[-11.1, 0, -13.6]} height={8.6} size={3.25} map={stone} lite={lite} />
      <SquareTower position={[11.1, 0, -13.6]} height={8.9} size={3.4} map={stone} lite={lite} />
      <SquareTower position={[-11.1, 0, 6.85]} height={8} size={3.1} map={stone} lite={lite} />
      <SquareTower position={[11.1, 0, 6.85]} height={8.2} size={3.15} map={stone} lite={lite} />
      <SquareTower position={[0, 0, -13.6]} height={6.4} size={2.35} map={stone} lite={lite} />
      {!lite && (
        <>
      ...
      <RoundTower position={[-2.4, 0, -2.6]} height={9.6} radius={1.85} map={stone} lite={lite} />
      <RoundTower position={[4.8, 0, -8.8]} height={7.4} radius={1.5} map={stone} lite={lite} />
      <RoundTower position={[-5.6, 0, -8.4]} height={6.8} radius={1.25} map={stone} lite={lite} />

      <group position={[1.2, 0, -1.2]}>
        <Block args={[6.4, 12.4, 5.8]} position={[0, 6.2, 0]} color={STONE} map={stone} lite={lite} />
        <Block args={[7, 0.42, 6.4]} position={[0, 12.55, 0]} color={STONE_3} map={stone} lite={lite} />
        <Block args={[4.2, 2.8, 3.8]} position={[0, 13.95, 0]} color={STONE_2} map={stone} lite={lite} />
        <mesh position={[0, 16.1, 0]}>
          <coneGeometry args={[2.55, 2.4, lite ? 5 : 8]} />
          {lite ? <meshBasicMaterial color="#5c2a22" /> : <meshStandardMaterial color="#5c2a22" roughness={0.78} />}
        </mesh>
        {!lite && (
          <>
      ...
      <Block args={[4.6, 3.4, 3.8]} position={[-5.2, 1.7, -5.4]} color={STONE_2} map={stone} lite={lite} />
      <Block args={[5, 0.22, 4.2]} position={[-5.2, 3.45, -5.4]} color={STONE_3} map={stone} lite={lite} />
      <mesh position={[-5.2, 4.55, -5.4]} rotation={[0, 0.2, 0]}>
        <coneGeometry args={[2.6, 1.8, 4]} />
        {lite ? <meshBasicMaterial color="#6a3226" /> : <meshStandardMaterial color="#6a3226" roughness={0.8} />}
      </mesh>
